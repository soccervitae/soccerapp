...
AS $function$
BEGIN
  -- Delete user's highlights and ALL related data
  ...
  -- Delete user's stories and ALL related data
  DELETE FROM story_reports WHERE story_id IN (SELECT id FROM stories WHERE user_id = p_user_id);
  ...
  -- Delete user's posts and ALL related data
  ...
  -- Delete user's interactions on OTHER people's content
  ...
  DELETE FROM story_reports WHERE reporter_id = p_user_id;
  ...
  -- Clean up empty conversations (no participants left)
  ...
  DELETE FROM support_ticket_replies WHERE ticket_id IN (SELECT id FROM support_tickets WHERE user_id = p_user_id);
  DELETE FROM support_ticket_replies WHERE user_id = p_user_id;
  ...
  -- Finally, delete from auth.users to remove authentication record
