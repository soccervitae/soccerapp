...
export const ProfileInfo = ({ profile, followStats, isOwnProfile = false }: ProfileInfoProps) => {
  ...
  const handleFollowClick = () => {
    ...
    followUser.mutate(
      { userId: profile.id, isFollowing: isCheering },
  ...
  return (
    <section className="flex flex-col items-center gap-4">
      ...
      <div className="w-full h-32 relative overflow-hidden">
        <img
          src={profile.cover_url || "https://images.unsplash.com/photo-1574629810360-7efbbe195018?w=800&h=300&fit=crop"}
          alt="Cover photo"
      ...
      <div className="text-center space-y-1">
        ...
        <p className="text-muted-foreground font-medium text-sm">
          {profile.position && profile.team 
            ? `${profile.position} | ${profile.team}` 
            : profile.position || profile.team || `@${profile.username}`}
        </p>
        {profile.bio && (
          <p className="text-muted-foreground text-xs italic px-6 line-clamp-2">{profile.bio}</p>
        )}
      </div>
        ...
        {isOwnProfile ? (
          ...
          <>
            <button 
              ...
              className={`flex-1 h-9 rounded font-semibold text-xs tracking-wide transition-all duration-300 flex items-center justify-center gap-1.5 disabled:opacity-50 ${
                isCheering 
