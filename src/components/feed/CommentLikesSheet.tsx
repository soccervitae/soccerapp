...
import { useCommentLikers } from "@/hooks/useCommentLikes";
...
interface CommentLikesSheetProps {
  commentId: string;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export const CommentLikesSheet = ({ commentId, open, onOpenChange }: CommentLikesSheetProps) => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const { data: likers, isLoading } = useCommentLikers(commentId, open);
  ...
  return (
    <ResponsiveModal open={open} onOpenChange={onOpenChange}>
      <ResponsiveModalContent className="h-[60vh] sm:max-w-lg flex flex-col z-[80]" overlayClassName="z-[80]">
        <ResponsiveModalHeader className="border-b border-border pb-3">
          <ResponsiveModalTitle className="text-center">Aplausos</ResponsiveModalTitle>
        </ResponsiveModalHeader>
