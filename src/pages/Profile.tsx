...
import { useProfile, useProfileByUsername, useFollowStats, useProfileView, useUserHighlights } from "@/hooks/useProfile";
...
const Profile = () => {
  ...
  const {
    ...
    isFetching: usernameFetching,
    refetch: refetchProfileByUsername
  ...
  } = useProfile(!username ? user?.id : undefined);
  ...
  } = useFollowStats(targetUserId);
  const {
    data: highlights,
    isLoading: highlightsLoading
  } = useUserHighlights(targetUserId);
  const {
    data: championships,
    isLoading: championshipsLoading
  } = useUserChampionships(targetUserId);
  const {
    data: achievements,
  ...
  const photoPosts = userPosts?.filter(post => post.media_type === 'image' || post.media_type === 'carousel') || [];
  ...
  if (isLoading) {
    return <main className="bg-background min-h-screen relative pb-24">
        <div className="pt-16">
          <ProfileSkeleton />
        </div>
  ...
  if (!profile) {
    return <main className="bg-background min-h-screen relative pb-24 flex items-center justify-center">
  ...
  const renderProfileFeed = () => {
    if (postsLoading) {
      return <div className="grid grid-cols-3 gap-1">
          {[...Array(9)].map((_, i) => <div key={i} className="aspect-square bg-muted animate-pulse rounded-sm" />)}
        </div>;
    }
    ...
    return <div className="grid grid-cols-3 gap-px">
        {userPosts.map((post, index) => {
        const mediaUrls = post.media_url?.split(',') || [];
        const firstMedia = mediaUrls[0];
        const isVideo = post.media_type === 'video';
        const isCarousel = post.media_type === 'carousel' && mediaUrls.length > 1;
        return <div key={post.id} ref={el => {
          gridItemRefs.current[index] = el;
        }} className="aspect-square relative overflow-hidden bg-muted cursor-pointer" onClick={() => handleGridItemClick(index, gridItemRefs.current[index])} style={{
          WebkitTapHighlightColor: 'transparent'
        }}>
              {isVideo ? <video src={firstMedia} className="w-full h-full object-cover" muted playsInline /> : <img src={firstMedia} alt="" className="w-full h-full object-cover" loading="lazy" />}
              {isVideo && <div className="absolute top-2 right-2">
                  <span className="material-symbols-outlined text-white text-[18px] drop-shadow-lg">play_circle</span>
                </div>}
              {isCarousel && <div className="absolute top-2 right-2">
  ...
  const renderMediaGrid = (posts: typeof userPosts, emptyMessage: string, emptyIcon: string) => {
    if (postsLoading) {
      return <div className="grid grid-cols-3 gap-1">
          {[...Array(6)].map((_, i) => <div key={i} className="aspect-square bg-muted animate-pulse rounded-sm" />)}
        </div>;
    }
    ...
    return <div className="grid grid-cols-3 gap-1">
        ...
        const isVideo = post.media_type === 'video';
        const isCarousel = post.media_type === 'carousel' && mediaUrls.length > 1;
        return <div key={post.id} className="aspect-square relative overflow-hidden rounded-sm bg-muted">
              {isVideo ? <video src={firstMedia} className="w-full h-full object-cover" muted playsInline /> : <img src={firstMedia} alt="" className="w-full h-full object-cover" loading="lazy" />}
              {isVideo && <div className="absolute top-2 right-2">
                  <span className="material-symbols-outlined text-white text-[18px] drop-shadow-lg">play_circle</span>
                </div>}
  ...
  const ProfileTabs = () => <Tabs value={activeTab} onValueChange={setActiveTab} className={`w-full ${hasHighlights ? 'mt-6' : 'mt-2'}`}>
      <TabsList className="w-full h-auto bg-transparent p-0 border-b border-border flex justify-between">
        <TabsTrigger value="profile" className="flex-1 flex-col gap-0.5 text-xs py-2 rounded-none bg-transparent data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary text-muted-foreground">
          <span className="material-symbols-outlined text-[20px]">grid_on</span>
          Posts
        </TabsTrigger>
        <TabsTrigger value="videos" className="flex-1 flex-col gap-0.5 text-xs py-2 rounded-none bg-transparent data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary text-muted-foreground">
          <span className="material-symbols-outlined text-[20px]">play_circle</span>
          Vídeos
        </TabsTrigger>
        <TabsTrigger value="championships" className="flex-1 flex-col gap-0.5 text-xs py-2 rounded-none bg-transparent data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary text-muted-foreground">
          <span className="material-symbols-outlined text-[20px]">sports_soccer</span>
          Campeonatos
        </TabsTrigger>
        <TabsTrigger value="achievements" className="flex-1 flex-col gap-0.5 text-xs py-2 rounded-none bg-transparent data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary text-muted-foreground">
          <span className="material-symbols-outlined text-[20px]">trophy</span>
          Conquistas
        </TabsTrigger>
        <TabsTrigger value="photos" className="flex-1 flex-col gap-0.5 text-xs py-2 rounded-none bg-transparent data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary text-muted-foreground">
      ...
      if (info.offset.x < -50) {
        ...
        <TabsContent value="championships" className="mt-4" forceMount={activeTab === "championships" ? true : undefined}>
          {activeTab === "championships" && <ChampionshipsTab championships={championships || []} isLoading={championshipsLoading} isOwnProfile={isOwnProfile} />}
        </TabsContent>

        <TabsContent value="achievements" className="mt-4" forceMount={activeTab === "achievements" ? true : undefined}>
          {activeTab === "achievements" && <AchievementsTab achievements={achievements || []} isLoading={achievementsLoading} isOwnProfile={isOwnProfile} />}
        </TabsContent>
  ...
  const MainContent = () => <>
      ...
      <div className="pt-12 flex flex-col gap-6 py-[12px]">
        <ProfileInfo profile={profile} followStats={followStats} isOwnProfile={isOwnProfile} />
        <div className="px-4">
          <HighlightsSection highlights={highlights || []} isLoading={highlightsLoading} isOwnProfile={isOwnProfile} profileUsername={profile.username} profileAvatarUrl={profile.avatar_url} />
        </div>
        <ProfileTabs />
      </div>
    </>;
  ...
  return <ContentWrapper {...wrapperProps as any}>
      {fromOnboarding ? <motion.div variants={containerVariants} initial="hidden" animate="visible">
        ...
        </motion.div> : <MainContent />}
      
      {user ? <BottomNavigation activeTab="profile" /> : <GuestBanner />}

      {/* Profile Feed Sheet */}
      {profile && userPosts && <ProfileFeedSheet posts={userPosts} initialPostIndex={selectedPostIndex} isOpen={feedSheetOpen} onClose={() => setFeedSheetOpen(false)} profile={{
      id: profile.id,
      username: profile.username,
