...
const Profile = () => {
  ...
  const transformToFeedPost = (post: NonNullable<typeof allInfinitePosts>[0]): Post => ({
    ...
    music_title: post.music_title || null,
    music_artist: post.music_artist || null,
    music_audio_url: post.music_audio_url || null,
    music_cover_url: post.music_cover_url || null,
    music_duration_seconds: post.music_duration_seconds ?? null,
    music_source: post.music_source || null,
  });
  ...
  const tabOrder = isTeamOrSchool
    ? (isGuest ? ["videos", "photos", "championships", "achievements"] : ["profile", "videos", "photos", "championships", "achievements"])
    : isOfficialAccount 
  ...
  const ProfileTabs = () => (
    <Tabs value={activeTab} onValueChange={changeTabPreservingScroll} className={`w-full ${hasHighlights ? 'mt-2' : 'mt-0'}`}>
      <TabsList data-profile-tabs-list="true" className={`w-full h-auto p-0 border-b border-border flex ${isMobile ? 'justify-center sticky top-[50px] z-30 bg-background' : 'justify-center bg-transparent'}`}>
        {!isGuest && (
          <TabsTrigger 
            value="profile" 
